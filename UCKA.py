...
def rodada_multihop(clusters, nos, viz, bateria, bs_pos, alpha=0.5, beta=0.5, metodo='peso', historico_chs=None):
    ...
    if metodo == 'peso':
        ...
        return novos_clusters, CHs_anteriores, CHs_novos, list(todos_mortos_rodada), None

    elif metodo == 'round_robin':
        novos_clusters, CHs_anteriores, CHs_novos, novo_historico = reeleicao_round_robin(
            clusters_para_reeleicao, historico_chs or {}
        )
...
def reeleicao_round_robin(clusters, historico_chs):
    """
    Reeleição de CHs com round-robin. Garante que todos os membros de um cluster
    serão usados antes de repetir algum.
    ...
    - CHs_anteriores: set
    - CHs_novos: set
    - novo_historico: dict atualizado com nova lista de CHs usados
    """
    novos_clusters = {}
    CHs_anteriores = set(clusters.keys())
    CHs_novos = set()
    ...
    for ch_antigo, membros in clusters.items():
        ...
        usados = historico_chs.get(ch_antigo, [])
        restantes = [n for n in candidatos if n not in usados]

        if not restantes:
            # Se todos já foram usados, reinicia o ciclo
            novo_ch = candidatos[0]
            novo_uso = [novo_ch]
        else:
            novo_ch = restantes[0]
            novo_uso = usados + [novo_ch]

        outros = [i for i in candidatos if i != novo_ch]
        novos_clusters[novo_ch] = outros
        CHs_novos.add(novo_ch)
        novo_historico[novo_ch] = novo_uso
...
def bateria_total_restante(bateria):
    """
    Retorna a soma da bateria de todos os nós vivos (> 0).
    """
...
def main(metodo_reeleicao):
    ...
    mortos_plotados = set()
    historico_chs = {} if metodo_reeleicao == 'round_robin' else None
    ...
    for rodada in range(1, max_rounds + 1):
        ...
        if metodo_reeleicao == 'round_robin':
            ...
            clusters, CHs_anteriores, CHs_novos, mortos_rodada, _ = rodada_multihop(
                clusters, nos, viz, bateria, bs_pos, alpha, beta, metodo='peso'
            )
