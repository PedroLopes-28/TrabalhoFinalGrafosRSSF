...
def salvar_dataset_csv(estatisticas, caminho_csv='espso_base_centro_round_robin.csv'):
...
class StaticSink:
    def __init__(self, posicao):
        ...
        self.trajetoria = [posicao]  # Apenas para compatibilidade com o restante do código
...
def encontrar_melhor_posicao_bs(coords, grid_size=50, margem=50):
    """
    Testa múltiplas posições candidatas para a base e escolhe a que minimiza
    a soma das distâncias euclidianas aos nós sensores.

    Parâmetros:
    - coords: lista de tuplas (x, y) dos nós sensores
    - grid_size: resolução da grade de busca (quanto menor, mais precisa)
    - margem: quanto expandir o espaço além da borda da rede

    Retorna: (x_bs, y_bs) ótimos
    """
    ...
    for xb in range(int(xmin), int(xmax) + 1, grid_size):
        for yb in range(int(ymin), int(ymax) + 1, grid_size):
            custo = sum(math.hypot(x - xb, y - yb) for x, y in coords)
            if custo < menor_custo:
                menor_custo = custo
                melhor_pos = (xb, yb)

    print(f"📍 Melhor posição da base encontrada: {melhor_pos} com custo {menor_custo:.2f}")
    return melhor_pos
...
def main(metodo_reeleicao='round_robin'):
    ...
    bs_pos = encontrar_melhor_posicao_bs(coords)
    posicao_inicial_sink = StaticSink(bs_pos)
    ...
    nos = inicializar_nos(coords, energia_inicial, posicao_inicial_sink)
    ...
    CHs_iniciais = eleger_CHs_com_PSO(nos, viz, alpha, beta, posicao_sink=posicao_inicial_sink.posicao)
    ...
    arestas_iniciais = coletar_arestas_intracluster(clusters, nos, posicao_inicial_sink.posicao)
    prev_CHs = set(CHs_iniciais)
    visualizar_pso_com_trajetoria(nos, prev_CHs, clusters, arestas_iniciais, posicao_inicial_sink, rodada=0)
    ...
    for rodada in range(1, max_rodadas + 1):
        ...
        for no in nos:
            no.distancia_bs = euclidiana((no.x, no.y), posicao_inicial_sink.posicao)
        ...
        if metodo_reeleicao == 'round_robin':
            clusters, CHs_anteriores, CHs_novos, mortos_rodada, historico_chs = rodada_multihop(
                clusters, nos, viz, bateria, posicao_inicial_sink.posicao, alpha, beta,
                metodo='round_robin', historico_chs=historico_chs
            )
        else:
            clusters, CHs_anteriores, CHs_novos, mortos_rodada, _ = rodada_multihop(
                clusters, nos, viz, bateria, posicao_inicial_sink.posicao, alpha, beta, metodo='peso'
            )
        ...
        if novos_mortos:
            arestas_atual = coletar_arestas_intracluster(clusters, nos, posicao_inicial_sink.posicao)
        ...
        if not clusters:
            visualizar_pso_com_trajetoria(nos, prev_CHs, clusters, arestas_atual, posicao_inicial_sink, rodada)
    ...
    if clusters:
        ...
        arestas_finais = coletar_arestas_intracluster(clusters, nos, posicao_inicial_sink.posicao)
        visualizar_pso_com_trajetoria(nos, ultimos_CHs, clusters, arestas_finais, posicao_inicial_sink, rodada)
